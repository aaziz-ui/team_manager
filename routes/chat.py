...
def unsave_from_project(msg_id):
    msg = ChatMessage.query.get_or_404(msg_id)
    if current_user.role != ROLE_ADMIN and msg.saved_by_id != current_user.id:
        abort(403)
    project_name = msg.saved_to_project.name if msg.saved_to_project else None
    msg.saved_to_project_id = None
    msg.saved_by_id = None
    db.session.commit()
    flash(f'Removed from "{project_name}" saved notes.' if project_name else "Removed from saved notes.", "success")
