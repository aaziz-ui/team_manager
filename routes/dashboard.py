...
from models import (
    db, User, Task, TaskCollaborator, Vacation,
...
CHART_RADIUS = 40
CHART_CIRCUMFERENCE = 2 * pi * CHART_RADIUS
STATUS_ORDER = ["available", "meeting", "idle", "away_desk", "offline"]


def build_status_chart(employees):
    """Returns (segments, total) for an SVG donut chart of the team's current live status mix.
    Each segment carries a stroke-dasharray/dashoffset pair ready to render directly."""
    counts = {}
    for e in employees:
        counts[e.display_status] = counts.get(e.display_status, 0) + 1
    ...
    for status in STATUS_ORDER:
        ...
        if not count:
            continue
        length = (count / total) * CHART_CIRCUMFERENCE
        segments.append({
            "status": status,
            "label": STATUS_LABELS[status],
            "count": count,
            "dasharray": f"{length:.2f} {CHART_CIRCUMFERENCE:.2f}",
            "dashoffset": f"{-cumulative:.2f}",
        })
        cumulative += length
    return segments, len(employees)
...
def index():
    ...
    if current_user.role == "admin":
        reports = [e for e in employees if e.role in ("manager", "employee") and e.id != current_user.id]
    elif current_user.role == "manager":
        reports = [e for e in employees if e.role == "employee" and current_user.can_manage(e)]
    else:
        reports = []

    for r in reports:
        r_collab_task_ids = {c.task_id for c in TaskCollaborator.query.filter_by(user_id=r.id).all()}
        open_tasks = Task.query.filter(
            db.or_(Task.assigned_to_id == r.id, Task.id.in_(r_collab_task_ids)),
    ...
    chart_segments, chart_total = build_status_chart(employees)
    ...
    return render_template(
        ...
        chart_circumference=CHART_CIRCUMFERENCE,
    )
