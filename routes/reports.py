...
def mark_read(report_id):
    ...
    db.session.commit()
    notify(report.user_id, "report",
           f'{current_user.full_name} read your report for {report.report_date.strftime("%b %d")} - it\'s now locked until they mark it unread',
           url=url_for("reports.index", date=report.report_date.isoformat(), _anchor=f"report-{report.id}"))
    db.session.commit()
...
def mark_unread(report_id):
    ...
    db.session.commit()
    notify(report.user_id, "report",
           f'{current_user.full_name} marked your report for {report.report_date.strftime("%b %d")} as unread - you can edit it again',
           url=url_for("reports.index", date=report.report_date.isoformat(), _anchor=f"report-{report.id}"))
    db.session.commit()
