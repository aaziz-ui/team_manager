...
from models import db, User, Task, TaskComment, Vacation, DailyReport, StatusLog, ROLE_ADMIN, CompanySettings, STATUS_LABELS
from status_tracking import compute_daily_hours, seconds_to_hm, get_day_activity_window
...
def hours():
    ...
    for e in employees:
        ...
        rows.append({
            ...
            "started": first_seen.strftime("%H:%M") if first_seen else "—",
...
def reset_hours():
    ...
    deleted = (StatusLog.query.filter(
        StatusLog.started_at < day_end,
        db.or_(StatusLog.ended_at == None, StatusLog.ended_at > day_start),  # noqa: E711
    ).delete(synchronize_session=False))
    db.session.commit()

    flash(f"Cleared tracked hours for {day.strftime('%b %d, %Y')} ({deleted} record(s) removed).", "success")
    return redirect(url_for("admin.hours", date=day.isoformat()))
...
def hours_export_csv():
    ...
    writer.writerow(["Employee", "Started (PT)", "Ended (PT)", "Available", "Idle",
                      "Away from desk", "In a meeting", "Offline"])
    for e in employees:
        totals = compute_daily_hours(e, day)
        first_seen, last_seen = get_day_activity_window(e, day)
        writer.writerow([
            e.full_name,
            first_seen.strftime("%H:%M") if first_seen else "",
