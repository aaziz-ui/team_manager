...
from models import (
    ...
    ChatMessage, ChatReaction, ChatReadMarker, TaskCollaborator, TaskWatcher, Note, NoteShare, NewsPost,
...
def delete_user(user_id):
    ...
    try:
        ...
        # Company news posts have organizational value that outlives whoever wrote them -
        # keep the announcement, just detach the author link (nullable, same treatment as
        # project activity entries) rather than deleting the post itself.
        NewsPost.query.filter_by(author_id=user.id).update({"author_id": None})
...
def employee_log():
    ...
    if employee:
        ...
        employee_collab_task_ids = [c.task_id for c in TaskCollaborator.query.filter_by(user_id=employee.id).all()]
        tasks_completed = Task.query.filter(
            db.or_(Task.assigned_to_id == employee.id, Task.id.in_(employee_collab_task_ids)),
