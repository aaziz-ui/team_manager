...
def draft_completed_task_into_report(task):
    ...
    today, creating that report if it doesn't exist yet. It's still just a normal editable
    draft afterward - they can add to it, remove the line, or change anything else about
    it freely, same as if they'd typed it themselves (unless a manager has already marked
    that report read and locked it, in which case we leave it alone rather than silently
    editing something that's already been reviewed)."""
    today = date.today()
    line = f"- Completed: {task.title}"
    report = DailyReport.query.filter_by(user_id=task.assigned_to_id, report_date=today).first()
    if report:
        ...
        db.session.add(DailyReport(user_id=task.assigned_to_id, report_date=today, content=line))
