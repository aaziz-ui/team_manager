...
def _run_lightweight_migrations():
    ...
    if "users" in existing_tables:
        ...
        if "status_locked" not in user_cols:
            ...
            db.session.commit()
        if "vacation_days_override" not in user_cols:
            db.session.execute(text(
                "ALTER TABLE users ADD COLUMN vacation_days_override INTEGER"
            ))
            db.session.commit()
        if "can_assign_tasks" not in user_cols:
            db.session.execute(text(
                "ALTER TABLE users ADD COLUMN can_assign_tasks BOOLEAN NOT NULL DEFAULT FALSE"
            ))
            db.session.commit()
    ...
    if "daily_reports" in existing_tables:
        ...
        if "is_read" not in dr_cols:
            db.session.execute(text(
                "ALTER TABLE daily_reports ADD COLUMN is_read BOOLEAN NOT NULL DEFAULT FALSE"
            ))
            db.session.commit()
        if "read_by_id" not in dr_cols:
            db.session.execute(text(
                "ALTER TABLE daily_reports ADD COLUMN read_by_id INTEGER"
            ))
            db.session.commit()
        if "read_at" not in dr_cols:
            db.session.execute(text(
                "ALTER TABLE daily_reports ADD COLUMN read_at DATETIME"
            ))
            db.session.commit()
