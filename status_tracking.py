...
def pacific_day_bounds_utc(day):
    """Returns (day_start, day_end) as naive UTC datetimes marking midnight-to-midnight of
    the given calendar day in PACIFIC time - not a naive UTC midnight. Using a naive UTC
    midnight here would actually land in the afternoon/evening of the PREVIOUS day once
    converted to Pacific for display (7-8 hours off depending on DST) - exactly the kind of
    bug that shows everyone "starting" at a nonsensical fixed time like 17:00 the day
    before, whenever an old, never-properly-closed status entry gets clamped to it."""
    start_local = datetime.combine(day, dtime.min).replace(tzinfo=PACIFIC)
    end_local = datetime.combine(day, dtime.max).replace(tzinfo=PACIFIC)
...
def compute_daily_hours(user, day, window_start=None, window_end=None):
    ...
    day_start, day_end = pacific_day_bounds_utc(day)
    ...
    # If they've been quiet since before the counting window even opened, none of that
    # silence belongs to a real, still-open session from today - it's simply Offline for
    # the whole window, full stop. Skip the log matching entirely so an old, never-closed
    # entry from a previous session can't get counted here at all.
    if offline_since <= count_start:
        if clip_end > count_start:
            totals[STATUS_OFFLINE] = (clip_end - count_start).total_seconds()
        return totals
...
def late_minutes(first_seen_pacific, settings):
    """Given a Pacific-local first_seen datetime and CompanySettings, returns how many whole
    minutes after official office opening the person's first activity was (0 if on time or
    early). Returns None if there's no activity to compare against."""
    if first_seen_pacific is None:
        return None
    work_start_today = first_seen_pacific.replace(
        hour=settings.work_start.hour, minute=settings.work_start.minute, second=0, microsecond=0)
    delta_minutes = (first_seen_pacific - work_start_today).total_seconds() / 60
    return max(0, round(delta_minutes))
...
def get_day_activity_window(user, day):
    ...
    If their session is still open (browser genuinely still active), "ended" reflects that
    ongoing activity. If they've gone quiet (no heartbeat for OFFLINE_THRESHOLD_MINUTES) but
    haven't reconnected yet to trigger the gap-backfill, this caps "ended" at the moment they
    actually went stale rather than the moment someone happens to be looking at this page -
    the same staleness logic used for the hours totals, kept consistent here."""
    day_start, day_end = pacific_day_bounds_utc(day)
    now = datetime.utcnow()
    ...
    if user.last_seen_at:
        ...
        offline_since = now  # never seen at all - treat as offline from the start

    # If they've been quiet since before this day even began, there's no genuine activity
    # of theirs ON this specific day at all - don't let an old, never-properly-closed status
    # entry from a previous session (still technically "open" because they haven't
    # reconnected since) get clamped into today's window and misread as "started at
    # midnight" (or worse, at some shifted hour from a UTC/Pacific boundary mismatch).
    if offline_since <= day_start:
        return None, None

    live_cutoff = offline_since if now > offline_since else now
    clip_end = min(day_end, live_cutoff)
    ...
    # Only treat this as a genuine check-in for TODAY if some activity actually began
    # today. A session that's simply continuing uninterrupted from a previous day (never
    # went stale, e.g. a workstation that never sleeps and stayed on the same status) isn't
    # a fresh start - there's nothing meaningful to report as "Started" even though the
    # person happens to be active right now.
    logs_starting_today = [l for l in logs if l.started_at >= day_start]
    if not logs_starting_today:
        return None, None

    first_start = logs_starting_today[0].started_at
    last_log = logs[-1]
    last_end = min(last_log.ended_at or live_cutoff, clip_end)
