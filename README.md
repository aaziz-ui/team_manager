...
- **Tasks** — managers/admins assign tasks (title, description, due date, priority) to
  ...
  ask a question or explain something). **Admins can delete any task.**
- **Daily hours report (admin only)** — Admin → Hours shows, per employee per day, how
  many hours were spent Available / Idle / Away from desk / In a meeting / Offline,
  built from a running log of status changes. Admin → Settings lets you set the
  company's expected working hours as a reference point.
- **Daily reports** — every employee can submit a quick end-of-day summary ("what I worked on today"). Their manager (or admin) sees it, along with who on the team hasn't submitted yet for a given day. Editable throughout the day; only the first submission triggers a notification to the manager, not every edit.
