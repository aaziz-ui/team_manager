...
- **Project dashboards** — a full project management view: client, project manager, and
  team; current phase and status (on track / at risk / delayed / on hold / completed);
  start date, target completion, and days remaining (or overdue); overall progress
  (auto-averaged from per-discipline engineering progress bars, e.g. Electrical 40%,
  Civil 60%); upcoming milestones; overdue tasks (any task can optionally be linked to a
  project from the Tasks page); a running activity feed; pinned notices; and tracked
  risks/issues with severity and status. Admins manage everything; a project's assigned
  manager can manage their own project; any team member can flag a risk or issue.
- **Team chat** — a company-wide general chat plus a dedicated chat for each project.
  Threaded replies, `@Full Name` mentions (which send a notification), emoji reactions,
  pinning (admin company-wide, or a project's manager within their own project), edit/
  delete (author or admin), search, and linking a message to a task. Any message can be
  **saved to a project** as a bookmarked note, showing up on that project's dashboard.
  Refreshes automatically every few seconds — near-real-time rather than instant, since
  true real-time (WebSockets) would need a bigger change to how the app is hosted.
  File/image attachments are supported, but stored on the app's local disk, which — like
  the database before you moved it to Postgres — is wiped on every redeploy unless you
  add persistent storage. Don't rely on attachments for anything you can't afford to lose.
...
- **Daily hours report (admin only)** — Admin → Hours shows, per employee per day, when they
  started and ended (in Pacific Time, like a time clock — flagged in red with the **exact number
  of minutes late** if they started after official office hours began), an "On-desk total"
  (Available + Idle + In a meeting combined), plus how many hours were spent in each individual
  status **within office hours only** (the counter starts when the office opens and stops when
  it closes, based on Admin → Settings). A search dropdown lets you jump straight to any one
  employee or manager's hours for any day. A **"Reset hours for this day"** button (with a
  confirmation prompt) lets an admin wipe tracked hours for a specific day — for everyone, or
  just the currently-selected employee.
- **Employee monthly log (admin only)** — Admin → Hours → "Employee monthly log" gives a
  per-employee summary for any month: total minutes late (and average per late day), total and
  average on-desk hours for the month plus a rolling this-week figure, vacation days taken (that
  month and year-to-date), tasks completed, and how many days had no tracked activity at all
  (useful for spotting gaps, cross-referenced against vacation days so it doesn't look like an
  unexplained absence when it's actually approved time off).
- **Daily reports** — every employee can submit a summary ("what I worked on today") for any date, not just today — write yesterday's report, catch up on a missed day, whatever's needed. A calendar/date picker lets anyone browse reports by date: employees see only their own, managers see their team's, admins see everyone's company-wide, along with who hasn't submitted yet for that day. Managers/admins can mark a report **read**, which locks it from further edits by the employee (shown to them as a small "Read by [name]" badge) until it's marked **unread** again. Only the first submission of a given day triggers a notification to the manager, not every edit.
...
- **Voice + popup alerts** — an "Alerts: off/on" toggle in the navbar lets anyone opt in to real browser desktop notifications and spoken voice alerts (via the browser's built-in Notification and Speech Synthesis APIs — no extra software) whenever they get a new notification, even if the Team Manager tab isn't focused. It's per-browser and off by default; turning it on asks for the browser's normal notification permission once. Clicking a popup jumps straight to what it's about, same as clicking the bell notification would.
...
- **Backup** — export users, tasks, reports, and vacations to a JSON file, and re-import
  it (merge or full replace) — useful for migrating between deployments or keeping
  offline backups. **Not yet included:** Projects and Chat data aren't part of this
  export/import yet — they're only in the live database, so make sure your database
  itself is on a plan that doesn't expire (see the hosting section below) rather than
  relying on this export for that data specifically.
