(function () {
  ...
  // ---- Near-real-time chat refresh (polls every few seconds) ----
  const chatMessages = document.getElementById("chat-messages");
  if (chatMessages) {
    setInterval(() => {
      const active = document.activeElement;
      if (active && chatMessages.contains(active)) return; // don't clobber someone mid-reply/edit
      fetch(window.location.pathname + window.location.search)
        .then((r) => r.text())
        .then((html) => {
          const doc = new DOMParser().parseFromString(html, "text/html");
          const fresh = doc.getElementById("chat-messages");
          if (fresh) chatMessages.innerHTML = fresh.innerHTML;
        })
        .catch(() => {});
    }, 6000);
  }

  // Unread badge for the Chat nav link (general chat only, for a lightweight nudge)
  ...
  if (chatNavBadge) {
    function refreshChatBadge() {
      fetch("/api/chat/unread")
        .then((r) => r.json())
        .then((data) => {
          chatNavBadge.textContent = data.count > 0 ? " (" + data.count + ")" : "";
        })
        .catch(() => {});
    }
    refreshChatBadge();
